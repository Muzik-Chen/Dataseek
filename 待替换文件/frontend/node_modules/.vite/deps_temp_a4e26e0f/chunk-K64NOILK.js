import {
  ...
  parseSanitizationFilter,
  passesSanitizationFilter,
  payloadDisableAnimation,
  ...
  scaleRawExtentInfoEnableBoxCoordSysUsage,
...
import {
  ...
  boundingRectCalculateTransform,
  boundingRectContain,
  boundingRectCopy,
  boundingRectCreate,
  clone,
  ...
  sub,
  transformableCreate,
  transformableGetLocalTransform,
  translate,
  trim
...
function canCalculateAffineTransform(scale2) {
  return (scale2.type === "interval" || scale2.type === "time") && !hasBreaks(scale2);
}
var Cartesian2D = (
  /** @class */
  (function(_super) {
  ...
  function deal(showMinMaxLabelOption, outmostLabelIdx, innerLabelIdx) {
    ...
    if (showMinMaxLabelOption == null) {
      ...
      if (
        ...
        isTimeScale(scale2) && tick.notNice || isOrdinalScale(scale2) && tick.offInterval
      ) {
...
function getCartesianAxisHashKey(axis) {
...
function scaleCalcAlign(targetAxis, alignToScale) {
  ...
  function updateMinFromMinNice() {
    min2 = round(minNice - interval * t0, intervalPrecision);
  }
  function updateMaxFromMaxNice() {
    max2 = round(maxNice + interval * t1, intervalPrecision);
  }
  function updateMinNiceFromMinT0Interval() {
    minNice = t0 ? round(min2 + interval * t0, intervalPrecision) : min2;
  }
  function updateMaxNiceFromMaxT1Interval() {
    maxNice = t1 ? round(max2 - interval * t1, intervalPrecision) : max2;
  }
  if (targetMinMaxFixed[0] && targetMinMaxFixed[1]) {
    min2 = targetExtent[0];
    max2 = targetExtent[1];
    interval = (max2 - min2) / (alignToNiceSegCount + t0 + t1);
    var axisPxExtent = targetAxis.getExtent();
    var pxSpan = mathAbs(axisPxExtent[1] - axisPxExtent[0]);
    intervalPrecision = getAcceptableTickPrecision([max2, min2], pxSpan, 0.5 / alignToNiceSegCount);
      ...
      loopIncreaseInterval(function() {
        ...
        if (currIntervalCount <= alignToNiceSegCount) {
          ...
          if (mayEnhanceZero && targetExtent[0] === 0) {
            ...
            moreCountPair = moreCount % 2 === 0 ? [lessHalfCount, lessHalfCount] : min2 + max2 < targetExtent[0] + targetExtent[1] ? [lessHalfCount, lessHalfCount + 1] : [lessHalfCount + 1, lessHalfCount];
          }
          ...
          updateMinFromMinNice();
          updateMaxFromMaxNice();
          if (min2 <= targetExtent[0] && max2 >= targetExtent[1]) {
            return true;
          }
        }
      });
  ...
  updateIntervalOrLogScaleForNiceOrAligned(targetScale, targetMinMaxFixed, targetOldIntervalExtent, [min2, max2], targetOldOutermostExtent, {
    ...
    Grid2.prototype.update = function(ecModel, api) {
      ...
      each(this._axesList, function(axis) {
        ...
        if (isOrdinalScale(scale2)) {
          scale2.setSortInfo(axis.model.get("categorySortInfo"));
        }
      });
...
function canOnZeroToAxis(onZeroOption, axis) {
  ...
  var scale2 = axis.scale;
  var kindEffective = getScaleValuePositionKind(scale2, 0, false);
...
var RADAR_DEFAULT_SPLIT_NUMBER = 5;
...
var RadarModel = (
  ...
  (function(_super) {
    ...
    RadarModel2.type = COMPONENT_TYPE_RADAR;
...
var VIEW_COORD_SYS_TRANS_RAW = 0;
var VIEW_COORD_SYS_TRANS_ROAM = 1;
var VIEW_COORD_SYS_TRANS_OVERALL = 2;
function inner2(viewCoordSys) {
  return viewCoordSys;
}
...
var View = (
  ...
  (function(_super) {
    ...
    function View2(invertY, legacyCenterBase, legacyGeo) {
      var _this = _super.call(this) || this;
      _this.type = VIEW_COORD_SYS_TYPE;
      _this.dimensions = ["x", "y"];
      var viewInner = inner2(_this);
    ...
    View2.prototype.getRoamTransform = function() {
      return transformableGetLocalTransform(inner2(this).trans[VIEW_COORD_SYS_TRANS_ROAM]);
    };
    View2.prototype.dataToPoint = function(data, noRoam, out) {
      var transform = noRoam ? inner2(this).mtRaw : inner2(this).mtOverall;
      out = out || [];
      return transform ? applyTransform(out, data, transform) : copy(out, data);
    };
    View2.prototype.pointToData = function(point, reserved, out) {
      out = out || [];
      var invTransform = inner2(this).mtOverallInv;
...
function viewCoordSysCopyOverallMatrix(out, viewCoordSys) {
  return copy2(out || [], inner2(viewCoordSys).mtOverall);
}
function viewCoordSysGetZoomOption(viewCoordSys) {
  return inner2(viewCoordSys).zoom;
}
function viewCoordSysCopyBoundingRect(out, viewCoordSys) {
  return boundingRectCopy(out || boundingRectCreate(), inner2(viewCoordSys).dataRect);
}
function viewCoordSysCopyViewRect(out, viewCoordSys) {
  return boundingRectCopy(out || boundingRectCreate(), inner2(viewCoordSys).viewRect);
}
function viewCoordSysCopyTrans(out, viewCoordSys, transKind) {
  return copyTransform(out || transformableCreate(), inner2(viewCoordSys).trans[transKind]);
}
...
function viewCoordSysSetRoamOptionFromModel(viewCoordSys, hostModel) {
  var viewInner = inner2(viewCoordSys);
...
function viewCoordSysSetBoundingRect(viewCoordSys, x, y, width, height) {
  var viewInner = inner2(viewCoordSys);
  viewInner.dataRect = new BoundingRect_default(x, y, width, height);
  if (viewCoordSysIsInputReady(viewInner)) {
    viewCoordSysUpdateTransform(viewInner);
  }
}
function viewCoordSysSetViewRect(viewCoordSys, x, y, width, height) {
  var viewInner = inner2(viewCoordSys);
  viewInner.viewRect = new BoundingRect_default(x, y, width, height);
  if (viewCoordSysIsInputReady(viewInner)) {
    viewCoordSysUpdateTransform(viewInner);
  }
}
...
function legacyCopyOverallTrans(target, overallTrans, mtOverall, mtOverallInv) {
  if (target) {
    ...
    copy2(target.transform || (target.transform = []), mtOverall);
    copy2(target.invTransform || (target.invTransform = []), mtOverallInv);
  }
}
...
function parseCenterOption(out, viewInner, centerOption) {
  ...
  if (lgCt) {
    set(out, parsePercent(centerOption[0], lgCt.w), parsePercent(centerOption[1], lgCt.h));
  } else if (dataRect) {
    set(out, parsePercent(centerOption[0], dataRect.width, dataRect.x), parsePercent(centerOption[1], dataRect.height, dataRect.y));
  }
  return true;
}
...
var Geo = (
  ...
  (function(_super) {
    ...
    function Geo2(name, map2, opt) {
      ...
      _this.view = new View_default(invertLongitute, useLegacyViewCoordSysCenterBase(opt.ecModel, opt.api), _this);
      _this.map = map2;
    ...
    Geo2.prototype.containPoint = function(point) {
      return this.view.containPoint(point);
    };
    Geo2.prototype.getArea = function(tolerance) {
      tolerance = tolerance || 0;
    ...
    Geo2.prototype.shouldClip = function() {
      return this._clip;
    };
    Geo2.prototype.getBoundingRect = function() {
      return this.view.getBoundingRect();
    };
    Geo2.prototype.getViewRect = function() {
      return this.view.getViewRect();
    };
    Geo2.prototype.getRoamTransform = function() {
...
var MapSeries = (
  ...
  (function(_super) {
    ...
    MapSeries2.prototype.__ownRoamView = function() {
      return mapSeriesNeedsDrawMap(this) ? this.coordinateSystem.view : null;
    };
    MapSeries2.type = "series." + SERIES_TYPE_MAP;
...
var GeoCreator = (
  ...
  (function() {
    ...
    GeoCreator2.prototype.create = function(ecModel, api) {
      ...
      each(buildAllMapSeriesGroups(ecModel, true), function(mapSeriesGroup, groupKey) {
        if (!mapSeriesGroupHasOwnGeo(groupKey)) {
          return;
        }
        var firstDeclaredMapSeries = mapSeriesGroup.r[0];
        var nameMapList = [];
        each(mapSeriesGroup.r, function(mapSeries) {
...
var GeoModel = (
  ...
  (function(_super) {
    ...
    GeoModel2.prototype.__ownRoamView = function() {
...
var tmpRectCII = new BoundingRect_default(0, 0, 0, 0);
...
function makeViewCoordSysActionType(mainType, subType, suffix) {
  return (mainType !== COMPONENT_MAIN_TYPE_SERIES ? mainType : subType === "map" ? "geo" : subType) + suffix;
}
...
function createViewCoordSysSimply(componentOrSeries, api, x, y, width, height, viewRect) {
  var viewCoordSys = new View_default(null, useLegacyViewCoordSysCenterBase(componentOrSeries.ecModel, api));
...
var MapDraw = (
  ...
  (function() {
    ...
    MapDraw2.prototype.__updateOnOwnRoam = function(mapOrGeoModel) {
      applyViewCoordSysTransToElement(this._transformGroup, VIEW_COORD_SYS_TRANS_ROAM, mapOrGeoModel.coordinateSystem.view, null);
    };
    MapDraw2.prototype._buildGeoJSON = function(viewCoordSys, api, geo, mapOrGeoModel, data, isVisualEncodedByVisualMap) {
    ...
    MapDraw2.prototype._buildSVG = function(viewCoordSys, api, geo, mapOrGeoModel, data, isVisualEncodedByVisualMap) {
      var mapName = geo.map;
    ...
    MapDraw2.prototype._enableBlurEntireSVG = function(focusSelf, mapOrGeoModel) {
...
var GeoView = (
  ...
  (function(_super) {
    ...
    GeoView2.prototype.__updateOnOwnRoam = function(payload, model, api) {
      var mapDraw = this._mapDraw;
      if (mapDraw) {
...
function install3(registers) {
  ...
  registers.registerAction({
    ...
    ecModel.eachComponent(makeQueryConditionKindA(payload, mainType, subType), function(componentOrSeries) {
      if (isSeries && !mapSeriesNeedsDrawMap(componentOrSeries)) {
        return;
      }
...
var ParallelModel = (
  ...
  (function(_super) {
    ...
    ParallelModel2.type = COMPONENT_TYPE_PARALLEL;
  ...
  (function() {
    ...
    Parallel2.prototype._init = function(parallelModel, ecModel, api) {
      ...
      each(dimensions, function(dim, idx) {
        ...
        var axisType = determineAxisType(axisModel);
        var axis = this._axesMap.set(dim, new ParallelAxis_default(dim, createScaleByModel(axisModel, axisType, false), [0, 0], axisType, axisIndex));
        axis.onBand = isAxisOnBand(axis.scale, axisModel);
        axis.inverse = axisModel.get("inverse");
        axisModel.axis = axis;
        axis.model = axisModel;
...
var Float64ArrayCtor = typeof Float64Array !== UNDEFINED_STR ? Float64Array : void 0;
...
// node_modules/echarts/lib/coord/axisStatisticsMetricsImpl.js
...
function metricLiPosMinGapImpl(ecModel, perKeyPerAxis, ecPreparePerKeyPerAxis) {
  ...
  var scale2 = axis.scale;
  var needTransform = scale2.needTransform();
  var filter2 = scale2.getFilter ? scale2.getFilter() : null;
  ...
  eachSeries(function(dimStoreIdx, seriesModel, store) {
    for (var i = 0, cnt = store.count(); i < cnt; ++i) {
      ...
      if (isFinite(val) && (!filter2 || passesSanitizationFilter(filterParsed, val))) {
        if (needTransform) {
          val = scale2.transformIn(val, null);
        }
  ...
  ecPreparePerKeyPerAxis.liPosMinGap = perKeyPerAxis.liPosMinGap = isNullableNumberFinite(min2) ? min2 : writeIdx > 0 ? LINEAR_POSITIVE_MIN_GAP_SINGLE_VALID_VALUE : LINEAR_POSITIVE_MIN_GAP_NO_VALID_VALUE;
...
// node_modules/echarts/lib/chart/helper/axisSnippets.js
...
// node_modules/echarts/lib/layout/barCommon.js
var SERIES_TYPE_BAR = "bar";
...
var COORD_SYS_TYPE_SINGLE_AXIS_COMPATIBLE = "singleAxis";
...
var LineDraw = (
  ...
  (function() {
    ...
    ;
    LineDraw2.prototype.incrementalUpdate = function(taskParams, lineData, incrementalId) {
      this._progressiveEls = [];
      function updateIncrementalAndHover(el2) {
...
function getThumbnailBridge(model) {
  if (model) {
    return inner4(model).bridge;
  }
}
function injectThumbnailBridge(model, thumbnailBridge) {
  if (model) {
    inner4(model).bridge = thumbnailBridge;
  }
}
...
export {
  ...
  COORD_SYS_TYPE_SINGLE_AXIS_COMPATIBLE,
  COMPONENT_TYPE_SINGLE_AXIS,
  AxisModel_default2 as AxisModel_default,
  install2,
  RoamController_default,
  VIEW_COORD_SYS_TRANS_RAW,
  VIEW_COORD_SYS_TRANS_OVERALL,
  viewCoordSysCopyOverallMatrix,
  viewCoordSysGetZoomOption,
  viewCoordSysCopyBoundingRect,
...
//# sourceMappingURL=chunk-K64NOILK.js.map
